...
describe('Login', () => {
  ...
  it('should present error state if form is invalid', () => {
    ...
    cy.getByTestId('email-wrap').should('have.attr', 'data-status', 'invalid')
    cy.getByTestId('email').should('have.attr', 'title', 'Invalid value.')
    cy.getByTestId('email-label').should('have.attr', 'title', 'Invalid value.')
    cy.getByTestId('password').focus().type(faker.random.alphaNumeric(3))
    cy.getByTestId('password-wrap').should('have.attr', 'data-status', 'invalid')
    cy.getByTestId('password').should('have.attr', 'title', 'Invalid value.')
    cy.getByTestId('password-label').should('have.attr', 'title', 'Invalid value.')
    cy.getByTestId('submit').should('have.attr', 'disabled')
    cy.getByTestId('error-wrap').should('not.have.descendants')
  })
